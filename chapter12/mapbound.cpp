...
int main()
{
    ...
    if ((it = object.lower_bound("one1")) != object.end())
        cout << "lower-bound `one1' is available, it is: " <<
                it->first << '\n';
    ...
    if ((it = object.upper_bound("one")) != object.end())
        cout << "upper-bound `one' is available, it is: " <<
                it->first << '\n';
