...
// IMPORTANT !!!!!!!!!!!!!!!!!!
// However, members of the friend class may already be used, even though the compiler hasn't seen 
// the friend class's interface yet:

//CLASS2
    template <typename Key, typename Value>
    //template <typename Key2, typename Value2>
    Iterator<Key, Value> Dictionary<Key, Value>::begin()
    {
...
//CLASS3a
    ...
    class Iterator
    {
        ...
        friend Iterator<Key, Value> Dictionary<Key, Value>::begin();
        std::map<Key, Value> &d_dict;
        Iterator(Dictionary<Key, Value> &dict);
        public:
//=
